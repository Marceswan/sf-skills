...
Preview LWC components locally with hot reload, no deployment needed. Run the commands in `scripts/local-dev-preview.sh`, or use the `sf lightning dev` commands directly:
...
### Hooks
- `hooks/scripts/lwc-lsp-validate.py` - LSP-based syntax validation for LWC files
- `hooks/scripts/validate_slds.py` - SLDS 2 / styling-hook compliance check
- `hooks/scripts/slds_linter_wrapper.py` - wrapper around the SLDS linter
- `hooks/scripts/template_validator.py` - HTML template anti-pattern validation
- `hooks/scripts/post-tool-validate.py` - post-write validation orchestration
- `hooks/scripts/slds_data/` - SLDS reference data (valid classes, styling hooks, deprecated patterns)
