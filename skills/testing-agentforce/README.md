...
npx skills add Jaganpro/sf-skills --skill testing-agentforce
```
...
|---------------|-------------|
| building-agentscript | Create/fix agent scripts (recommended) |
| developing-agentforce | Agentforce platform setup (Agent Builder, GenAi metadata) |
| configuring-connected-apps | OAuth setup for live preview |
| handling-sf-data | Generate test data for actions |
| generating-flow | Fix failing Flow actions |
| debugging-apex-logs | Analyze agent error logs |
...
3. Call building-agentscript to generate fixes
