...
|-------|-------|----------|
| DSL compilation output | `building-agentscript` | `references/instruction-resolution.md` § "What the LLM Actually Receives" |
| Programmatic trace access | `building-agentscript` | `references/debugging-guide.md` § "Programmatic Trace Access via CLI" |
| Historical session data (STDM) | `observing-agentforce` | `SKILL.md` — Data Cloud extraction pipeline |
| Builder trace architecture | `observing-agentforce` | `references/builder-trace-api.md` — v1.1 endpoint discovery |
