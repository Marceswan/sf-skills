...
└── (other agent commands in building-agentscript)
```
...
These subcommands enable automated conversation testing outside the interactive REPL. See also: `building-agentscript/references/cli-guide.md` for the full preview workflow.
...
|------------|--------------|
| Custom Agent (you control) | Fix agent via developing-agentforce |
...
|---------|-------|---------|
| `sf agent publish authoring-bundle` | building-agentscript | Publish agent before testing |
| `sf agent validate authoring-bundle` | building-agentscript | Validate agent syntax |
| `sf agent activate` | building-agentscript | Activate for preview |
