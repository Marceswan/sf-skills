<!-- Parent: testing-agentforce/SKILL.md -->
# Agentic Fix Loops
...
Agentic fix loops enable automated test-fix cycles: when agent tests fail, the system analyzes failures, generates fixes via building-agentscript skill, re-publishes the agent, and re-runs tests.
...
│  4. Generate fix using building-agentscript skill                   │
...
| **Fixing** | Apply fix via building-agentscript | Re-validate agent |
...
|----------------|------------|-------------------|---------------|
| `TOPIC_NOT_MATCHED` | Topic description doesn't match utterance | Add keywords to topic description | building-agentscript |
| `ACTION_NOT_INVOKED` | Action description not triggered | Improve action description, add explicit reference | building-agentscript |
| `WRONG_ACTION_SELECTED` | Wrong action chosen | Differentiate descriptions, add `available when` | building-agentscript |
| `ACTION_INVOCATION_FAILED` | Flow/Apex error during execution | Delegate to generating-flow or generating-apex | generating-flow / generating-apex |
| `GUARDRAIL_NOT_TRIGGERED` | System instructions permissive | Add explicit guardrails to system instructions | building-agentscript |
| `ESCALATION_NOT_TRIGGERED` | Missing escalation action | Add escalation to topic | building-agentscript |
| `RESPONSE_QUALITY_ISSUE` | Instructions lack specificity | Add examples to reasoning instructions | building-agentscript |
| `ACTION_OUTPUT_INVALID` | Flow returns unexpected data | Fix Flow or data setup | generating-flow / handling-sf-data |
| `TOPIC_RE_MATCHING_FAILURE` | Agent stays on old topic after user switches intent | Add transition phrases to target topic classificationDescription | building-agentscript |
| `CONTEXT_PRESERVATION_FAILURE` | Agent forgets info from prior turns | Add "use context from prior messages" to topic instructions | building-agentscript |
| `MULTI_TURN_ESCALATION_FAILURE` | Agent doesn't escalate after sustained frustration | Add frustration detection to escalation trigger instructions | building-agentscript |
| `ACTION_CHAIN_FAILURE` | Action output not passed to next action in sequence | Verify action output variable mappings and topic instructions | building-agentscript |
...
**Auto-Fix Command:**
```bash
Skill(skill="building-agentscript", args="Fix topic 'billing_inquiry' in agent MyAgent - add keywords: charged, invoice, payment")
```
...
**Auto-Fix Command:**
```bash
Skill(skill="building-agentscript", args="Fix action 'get_order_status' - improve description to trigger on 'where is order' utterances")
```
...
**Auto-Fix Command:**
```bash
Skill(skill="building-agentscript", args="Differentiate actions 'create_general_case' and 'create_technical_case' - add specific keywords to each")
```
...
```bash
# If Flow error
Skill(skill="generating-flow", args="Fix flow 'Get_Order_Status' - add input validation for order number format")

# If test data missing
Skill(skill="handling-sf-data", args="Create test order with number 12345 for agent testing")

# If Apex error
Skill(skill="generating-apex", args="Fix Apex class 'OrderLookupController' - handle invalid order numbers")
```
...
**Auto-Fix Command:**
```bash
Skill(skill="building-agentscript", args="Add guardrail to agent MyAgent - reject requests to delete or modify customer records")
```
...
**Auto-Fix Command:**
```bash
Skill(skill="building-agentscript", args="Add escalation trigger to agent MyAgent - escalate when user shows frustration")
```
...
**Auto-Fix Command:**
```bash
Skill(skill="building-agentscript", args="Fix topic 'reschedule' in agent MyAgent - add transition phrases: 'actually reschedule instead', 'change to reschedule'")
```
...
**Auto-Fix Command:**
```bash
Skill(skill="building-agentscript", args="Add context retention instructions to agent MyAgent - 'Always use information from prior messages, never re-ask for data already provided'")
```
...
**Auto-Fix Command:**
```bash
Skill(skill="building-agentscript", args="Add escalation triggers to agent MyAgent - detect 'nothing works', 'need a human', 'already tried that' as escalation signals")
```
...
**Auto-Fix Command:**
```bash
Skill(skill="building-agentscript", args="Fix action chaining in agent MyAgent - ensure GetCases uses AccountId from prior IdentifyRecord action output")
```
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  building-agentscript                                              │
│  └─ Create agent script → Validate → Publish                    │
│                    │                                             │
│                    ▼                                             │
│  testing-agentforce (this skill)                          │
...
│         │       ┌───────────┴───────────┐                       │
│         │       ▼                       ▼                       │
│         │   building-agentscript     generating-flow/generating-apex               │
...
|----------|---------------|-----------------|
| Fix agent script | building-agentscript | `Skill(skill="building-agentscript", args="Fix topic 'billing' - add keywords")` |
| Create test data | handling-sf-data | `Skill(skill="handling-sf-data", args="Create test Account with order data")` |
| Fix failing Flow | generating-flow | `Skill(skill="generating-flow", args="Fix flow 'Get_Order_Status' - add validation")` |
| Fix Apex error | generating-apex | `Skill(skill="generating-apex", args="Fix Apex class 'OrderController'")` |
| Setup ECA | configuring-connected-apps | `Skill(skill="configuring-connected-apps", args="Create External Client App for Agent Runtime API testing")` |
| Analyze debug logs | debugging-apex-logs | `Skill(skill="debugging-apex-logs", args="Analyze apex-debug.log from agent test")` |
...
│         │                │                                          │
│         ▼                ▼                                          │
│   Report Generator  +  Agentic Fix Loop (building-agentscript)        │
│                                                                     │
└────────────────────────────────────────────────────────────────────┘
```
...
❌ test_complex_order_inquiry
   ...
   🔧 Suggested Fix:
   Skill(skill="building-agentscript", args="Fix action 'get_order_status' in Coffee_Shop_FAQ_Agent - add handling for multiple order numbers in single utterance")
...
❌ test_edge_case_empty_input
   ...
   🔧 Suggested Fix:
   Skill(skill="building-agentscript", args="Add empty input handling to Coffee_Shop_FAQ_Agent system instructions")
```
...
python3 ~/.claude/plugins/cache/sf-skills/.../testing-agentforce/hooks/scripts/run-automated-tests.py \
  --agent-name MyAgent \
  --agent-file /path/to/MyAgent.agent \
  --target-org dev

# Generate spec only
python3 ~/.claude/plugins/cache/sf-skills/.../testing-agentforce/hooks/scripts/generate-test-spec.py \
  --agent-file /path/to/MyAgent.agent \
...
**Step 3: Generate Fix**
```bash
Skill(skill="building-agentscript", args="Fix topic 'billing_inquiry' in agent MyAgent - add keywords: charged, charge, payment to description")
```

**Step 4: Re-Publish Agent**
```bash
# developing-agentforce skill will:
...
|--------------|-------------|
| Agent script issues | building-agentscript |
| Flow execution errors | generating-flow |
| Apex exceptions | generating-apex |
| Debug log analysis | debugging-apex-logs |
| Test data issues | handling-sf-data |
