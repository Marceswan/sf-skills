...
```
Skill(skill="testing-agentforce", args="Generate tests for uncovered topic 'returns' in agent Customer_Support_Agent")
```
...
```
Skill(skill="testing-agentforce", args="Generate 5 alternative phrasings for topic 'order_lookup' - current phrasings: 'Where is my order?', 'Track my package'")
```
