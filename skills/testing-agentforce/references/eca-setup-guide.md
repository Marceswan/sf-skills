...
### Option 1: Use configuring-connected-apps Skill (Recommended)

```
Skill(skill="configuring-connected-apps", args="Create External Client App with Client Credentials flow for Agent Runtime API testing. Scopes: api, chatbot_api, sfap_api, refresh_token, offline_access")
```
...
python3 ~/.claude/skills/testing-agentforce/hooks/scripts/credential_manager.py \
...
1. AskUserQuestion: "Do you have an ECA with Client Credentials?"
   ...
   │
   └─ NO → Delegate:
           Skill(skill="configuring-connected-apps",
