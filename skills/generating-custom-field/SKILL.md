...
description: "Use this skill when users need to create, generate, or validate Salesforce Custom Field metadata. TRIGGER when users mention custom fields, field types, Roll-up Summary fields, Master-Detail relationships, Lookup relationships, formula fields, picklists, .field-meta.xml files, or field deployment errors (Roll-up Summary format, Master-Detail constraints, formula issues). DO NOT TRIGGER when: defining the parent object itself (use generating-custom-object), writing validation rules (use generating-validation-rule), granting field-level security via permission sets (use generating-permission-set), or deploying metadata (use deploying-metadata)."
license: MIT
metadata:
  version: "1.2"
...
## 9. Field-Level Security Follow-Up (Default)

Creating a custom field grants no visibility on its own. Object CRUD does NOT expose custom fields - field-level security (FLS) is the hidden blocker after deployment.

- Unless the user explicitly opts out, plan a Permission Set follow-up for every new custom field and include `fieldPermissions` for each eligible field rather than leaving FLS as a manual afterthought.
- Required fields and Master-Detail fields are NOT eligible for `fieldPermissions` and will fail deployment if included. Formula fields cannot be `editable`.
- Hand the FLS work to [generating-permission-set](../generating-permission-set/SKILL.md). See [references/field-and-cli-reference.md](references/field-and-cli-reference.md) for CLI describe commands to confirm field existence and required status before writing FLS.
...
| File | Use For |
|------|---------|
| [references/field-types-guide.md](references/field-types-guide.md) | Choosing the right field type and its required attributes |
| [references/field-types-example.md](references/field-types-example.md) | Complete worked examples of every common field type |
| [references/field-and-cli-reference.md](references/field-and-cli-reference.md) | Field-type selection guide, relationships, and `sf` CLI describe/deploy commands |
| [assets/fields/](assets/fields/) | Copy-ready `.field-meta.xml` templates per type: text, textarea, number, currency, date, email, phone, url, checkbox, picklist, multi-select-picklist, formula, rollup-summary, lookup, master-detail |
