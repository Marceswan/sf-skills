...
# Flows for Agentforce (building-agentscript)

## When building-agentscript Requests a Flow

- building-agentscript will invoke generating-flow (this skill) to create Flows
- generating-flow creates the validated Flow XML
- deploying-metadata handles deployment to org
- **Action Definition registration required** (see below)
- Only THEN can building-agentscript use `flow://FlowName` targets
...
1. **Deploy Flow** to target org via deploying-metadata
...
Flow Created  →  Deployed to Org  →  Action Definition Created  →  Agent Can Use
     ↑               ↑                        ↑                         ↑
   generating-flow       deploying-metadata           Setup > Agentforce         @actions.MyAction
```
