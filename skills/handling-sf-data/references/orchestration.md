...
```
1. generating-apex   → Create trigger handler class
2. generating-flow   → Create record-triggered flow
...
|------------|------------|------|
| generating-apex | → handling-sf-data | "Create 251 Accounts for bulk testing" |
| generating-flow | → handling-sf-data | "Create Opportunities with StageName='Closed Won'" |
| running-apex-tests | → handling-sf-data | "Generate test records for test class" |

| From handling-sf-data | To Skill | When |
|--------------|----------|------|
| handling-sf-data | → generating-custom-object | "Describe Invoice__c" (discover object structure) |
...
```
generating-apex:  Creates TestDataFactory_Account.cls
          ↓
