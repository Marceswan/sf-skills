...
description: "Build, modify, debug, and deploy Agentforce agents via Agent Script and the Setup UI / Agent Builder path, with 100-point scoring. TRIGGER when: user creates, modifies, or asks about .agent files or aiAuthoringBundle metadata; configures Builder-managed agents, topics, or actions; works with GenAiFunction, GenAiPlugin, or GenAiPromptTemplate metadata; uses Prompt Builder or Models API in an agent context; changes agent behavior or conversation logic; designs actions, tools, subagents, or flow control; writes or reviews an Agent Spec; previews, debugs, deploys, publishes, or tests agents via Agent Script CLI (sf agent generate/preview/publish/test). DO NOT TRIGGER when: Apex development (use generating-apex), Flow building (use generating-flow), Prompt Template authoring outside an agent, Experience Cloud config, agent testing (use testing-agentforce), persona design (use designing-agentforce-persona), session telemetry (use observing-agentforce), or Salesforce CLI tasks unrelated to Agentforce."
license: Apache-2.0
...
6. **Validate compilation** - `sf agent validate authoring-bundle --json --api-name <Developer_Name>`
...
3. [Design & Agent Spec](references/agent-design-and-spec-creation.md) - subagent graph design, flow control patterns, Agent Spec production,
   backing logic analysis; Section 3 for environment prerequisites
4. [Subagent Map Diagrams](references/agent-subagent-map-diagrams.md) - Mermaid diagram conventions for visualizing the agent's subagent graph
5. [Agent User Setup & Permissions](references/agent-user-setup.md) - permission set assignment, object permissions, cross-subagent validation
6. [Metadata & Lifecycle](references/agent-metadata-and-lifecycle.md) - directory structure, bundle metadata; publish troubleshooting
7. [Validation & Debugging](references/agent-validation-and-debugging.md) - validate the agent compiles, preview to confirm behavior
...
2. [Design & Agent Spec](references/agent-design-and-spec-creation.md) - Agent Spec structure, flow control pattern recognition
3. [Subagent Map Diagrams](references/agent-subagent-map-diagrams.md) - Mermaid conventions for subagent graph visualization
4. [Metadata & Lifecycle](references/agent-metadata-and-lifecycle.md) - directory conventions, bundle metadata
...
5. **Validate compilation** - `sf agent validate authoring-bundle --json --api-name <Developer_Name>`
...
2. [Core Language](references/agent-script-core-language.md) - syntax,
   anti-patterns
3. [Design & Agent Spec](references/agent-design-and-spec-creation.md) - Agent Spec updates, backing logic analysis
4. [Validation & Debugging](references/agent-validation-and-debugging.md) - compilation diagnosis, preview workflow, session trace analysis
...
2. [Validation & Debugging](references/agent-validation-and-debugging.md) - error taxonomy, error-to-root-cause mapping
...
1. [Core Language](references/agent-script-core-language.md) - execution
   model, anti-patterns
2. [Design & Agent Spec](references/agent-design-and-spec-creation.md) - Agent Spec as behavioral baseline, gating patterns
3. [Validation & Debugging](references/agent-validation-and-debugging.md) - preview workflow, session trace analysis
...
1. **Validate compilation** - `sf agent validate authoring-bundle --json --api-name <Developer_Name>`
...
2. [Validation & Debugging](references/agent-validation-and-debugging.md) - compilation validation, preview workflow
3. [Metadata & Lifecycle](references/agent-metadata-and-lifecycle.md) - dependency management, deploy commands; publish troubleshooting
...
2. [Validation & Debugging](references/agent-validation-and-debugging.md) - compilation validation, preview workflow
3. [Metadata & Lifecycle](references/agent-metadata-and-lifecycle.md) - delete mechanics, rename mechanics, orphan cleanup
...
3. [Design & Agent Spec](references/agent-design-and-spec-creation.md) - Agent Spec as test coverage baseline
...
## Setup UI / Agent Builder Path

Some agents are maintained through the **Setup UI / Agent Builder** path rather than Agent Script `.agent` files: declarative topics, Builder-managed actions, `GenAiFunction` / `GenAiPlugin` metadata, Prompt Builder templates stored as `GenAiPromptTemplate` metadata, Models API usage from Apex, and custom Lightning types.

Use the Builder path when the user is:
...
For new code-first agent development with strong control over flow/state, prefer the Agent Script path documented above (see also **building-agentscript**).

### Two Agentforce Paths

| Path | Best fit |
|------|----------|
| Setup UI / Agent Builder | Declarative maintenance, existing Builder agents, metadata-driven action registration |
| Agent Script DSL | Code-first `.agent` authoring, deterministic routing, version-controlled agent logic |
...
Expanded workflow: [builder-workflow.md](references/builder-workflow.md)

### Builder Platform Rules

- **Topic quality matters.** Topic descriptions are routing instructions for the planner. They must be specific, scenario-based, and non-overlapping with sibling topics.
- **Actions are wrappers around real targets.** Flow is the safest default for Builder actions; Apex covers complex business logic via `@InvocableMethod`; Prompt Builder templates generate summaries / drafts / recommendations. All are registered via `GenAiFunction`.
- **Prompt Template vs GenAiPromptTemplate.** "Prompt Template" is the plain-English / UI term used in Prompt Builder. `GenAiPromptTemplate` is the current Metadata API type. Prefer the current source format `genAiPromptTemplates/*.genAiPromptTemplate-meta.xml`. Plan around the 5-input maximum and reference inputs with the current merge-field shape, e.g. `{!$Input:TargetRecord}`.
- **Supporting metadata deploys first.** Deploy in order: metadata/fields, Apex, Flows, then `GenAiPromptTemplate` / `GenAiFunction` / `GenAiPlugin`, then publish the agent.
- **Publish does not activate.** After publish, run `sf agent activate` separately. For automation, prefer `sf agent activate --api-name <AgentName> --version <n> --target-org <alias> --json`.

### Builder Metadata Guidance

- **GenAiFunction** registers a single callable action. Validate: target exists; target is active / deployable; input names match the target contract; output names match the target contract; capability text clearly says when the action should be used.
- **GenAiPlugin** groups related functions into one logical package.
- **GenAiPromptTemplate** is for generated content, not deterministic business rules. Use metadata type `GenAiPromptTemplate`, folder `genAiPromptTemplates/`, suffix `.genAiPromptTemplate-meta.xml`, content under `templateVersions`; use published template versions before wiring dependent actions.
- **Models API** is for Apex-driven AI orchestration rather than Builder-only actions.
- **Custom Lightning Types** are for richer structured input or output presentation.

See [metadata-reference.md](references/metadata-reference.md), [genaiprompttemplate.md](references/genaiprompttemplate.md), [models-api.md](references/models-api.md), and [custom-lightning-types.md](references/custom-lightning-types.md).
...
```text
generating-custom-object -> generating-apex -> generating-flow -> developing-agentforce -> deploying-metadata
```

| Requirement | Delegate to | Why |
|-------------|-------------|-----|
| Create / fix Flows | generating-flow | Action target creation and Flow validation |
| Create / fix Apex actions | generating-apex | `@InvocableMethod` and Apex correctness |
| Code-first `.agent` authoring | building-agentscript | Deterministic FSM design and Agent Script DSL |
| Persona / voice design | designing-agentforce-persona | Tone, register, and identity definition |
| Test the agent | testing-agentforce | Formal test execution and assertions |
| Analyze session telemetry | observing-agentforce | STDM / trace analysis from production |
| Deploy / publish | deploying-metadata | Deployment orchestration |

## High-Signal Failure Patterns

| Symptom | Likely cause | Read next |
|---------|--------------|-----------|
| Action not available in Builder | target metadata missing or not deployed | [metadata-reference.md](references/metadata-reference.md) |
| Prompt action fails during publish or activation | template is Draft, missing inputs, or old metadata shape | [genaiprompttemplate.md](references/genaiprompttemplate.md) |
| Need more than 5 template inputs | flex template input limit hit | [genaiprompttemplate.md](references/genaiprompttemplate.md) |
| Apex AI logic times out | Models API work placed in the wrong context | [models-api.md](references/models-api.md) |
| Rich input/output UI not rendering | Lightning type config or prerequisites incomplete | [custom-lightning-types.md](references/custom-lightning-types.md) |
| Agent publishes but is not usable | forgot explicit activation | [cli-commands.md](references/cli-commands.md) |
...
Full rubric: [scoring-rubric.md](references/scoring-rubric.md).

## Reference File Index

| File | Contents |
|------|----------|
| [salesforce-cli-for-agents.md](references/salesforce-cli-for-agents.md) | CLI command syntax for generate, validate, deploy, publish, activate; Einstein Agent User creation |
| [agent-script-core-language.md](references/agent-script-core-language.md) | Execution model, syntax, block structure, anti-patterns |
| [agent-design-and-spec-creation.md](references/agent-design-and-spec-creation.md) | Subagent graph design, flow control, Agent Spec production, backing logic analysis, environment prerequisites |
| [agent-subagent-map-diagrams.md](references/agent-subagent-map-diagrams.md) | Mermaid conventions for visualizing the subagent graph |
| [agent-user-setup.md](references/agent-user-setup.md) | Permission set assignment, object permissions, cross-subagent validation |
| [agent-metadata-and-lifecycle.md](references/agent-metadata-and-lifecycle.md) | Directory structure, bundle metadata, publish troubleshooting, delete/rename mechanics |
| [agent-validation-and-debugging.md](references/agent-validation-and-debugging.md) | Compile validation, preview workflow, session trace analysis, error taxonomy |
| [agent-access-guide.md](references/agent-access-guide.md) | End-user access permissions, visibility troubleshooting |
| [architecture-patterns.md](references/architecture-patterns.md) | Hub-and-spoke, verification gate, post-action loop |
| [complex-data-types.md](references/complex-data-types.md) | Lightning type mapping decision tree |
| [feature-validity.md](references/feature-validity.md) | Feature validity by context |
| [instruction-resolution.md](references/instruction-resolution.md) | 3-phase runtime instruction model |
| [safety-review-reference.md](references/safety-review-reference.md) | 7-category safety review framework |
| [discover-reference.md](references/discover-reference.md) | Target discovery CLI |
| [scaffold-reference.md](references/scaffold-reference.md) | Stub generation CLI |
| [deploy-reference.md](references/deploy-reference.md) | Deployment lifecycle, error recovery, CI/CD, rollback |
| [known-issues.md](references/known-issues.md) | Platform bugs; load only when errors persist after code fixes |
| [production-gotchas.md](references/production-gotchas.md) | Credit consumption, token limits, loop guardrails, reserved keywords, lifecycle hooks, VS Code workarounds |
| [examples.md](references/examples.md) | Complete inline agent examples |
| [minimal-examples.md](references/minimal-examples.md) | Minimal agent snippets |
| [actions-reference.md](references/actions-reference.md) | Action definitions, invocation, target contracts |
| [action-prompt-templates.md](references/action-prompt-templates.md) | Prompt-template-backed actions |
| [builder-workflow.md](references/builder-workflow.md) | Setup UI / Agent Builder workflow |
| [metadata-reference.md](references/metadata-reference.md) | GenAiFunction, GenAiPlugin, GenAiPromptTemplate metadata |
| [genaiprompttemplate.md](references/genaiprompttemplate.md) | GenAiPromptTemplate source format and Draft/activation gotchas |
| [prompt-templates.md](references/prompt-templates.md) | Prompt Builder terminology and template planning |
| [models-api.md](references/models-api.md) | Models API from Apex |
| [custom-lightning-types.md](references/custom-lightning-types.md) | Custom Lightning type input/output rendering |
| [cli-commands.md](references/cli-commands.md) | Builder-path agent CLI commands |
| [scoring-rubric.md](references/scoring-rubric.md) | Full 100-point scoring rubric |
| [version-history.md](references/version-history.md) | Skill version history |
