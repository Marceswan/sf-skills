...
description: "Generates and reviews Salesforce Apex code with 150-point scoring. TRIGGER when: user writes, reviews, or fixes Apex classes, triggers, test classes, batch/queueable/schedulable/invocable jobs, or touches .cls/.trigger files; or asks to create/refactor a service, selector, domain, DTO, utility, interface, abstract, exception, REST resource, or @AuraEnabled controller. DO NOT TRIGGER when: LWC JavaScript (use generating-lwc-components), Flow XML (use generating-flow), SOQL-only queries (use querying-soql), or non-Salesforce code."
license: MIT
metadata:
  version: "1.1"
...
|---|---|---|
| Apex tests / fix failures / coverage | `generating-apex-test` | generate and iterate on test classes |
| Run Apex tests against an org | `running-apex-tests` | execute and iterate on org test failures |
| Describe objects / fields first | `generating-custom-object` | avoid coding against wrong schema |
| Seed bulk or edge-case data | `handling-sf-data` | create realistic test datasets |
| Deploy to org | `deploying-metadata` | validation and deployment orchestration |
| Build Flow that calls Apex | `generating-flow` | declarative orchestration |
| Build LWC that calls Apex | `generating-lwc-components` | UI/controller integration |
...
This skill handles production `.cls`/`.trigger`/`.apex` issues only: compile/parse failures, deployment dependency errors, runtime governor-limit failures. For test execution, assertions, coverage, or `sf apex run test` failures, delegate to `generating-apex-test` and `running-apex-tests`.
...
| File | Purpose |
|---|---|
| `references/patterns-deep-dive.md` | In-depth Apex pattern catalog |
| `references/best-practices.md` | General Apex best practices |
| `references/security-guide.md` | CRUD/FLS, sharing, injection defense |
| `references/security-quick-reference.md` | Condensed security checklist |
| `references/bulkification-guide.md` | Collection-safe bulk patterns |
| `references/testing-guide.md` | Testing approach and coverage |
| `references/testing-patterns.md` | Concrete test class patterns |
| `references/triangle-pattern.md` | Service-Selector-Domain triangle |
| `references/code-review-checklist.md` | Evidence-based review checklist |
| `references/anti-patterns.md` | Apex anti-patterns to avoid |
| `references/llm-anti-patterns.md` | LLM-specific Apex mistakes |
| `references/code-smells-guide.md` | Code smell identification |
| `references/naming-conventions.md` | Naming rules and examples |
| `references/design-patterns.md` | Reusable design patterns |
| `references/solid-principles.md` | SOLID applied to Apex |
| `references/trigger-actions-framework.md` | TAF setup and usage |
| `references/automation-density-guide.md` | Choosing automation layers |
| `references/flow-integration.md` | Flow-to-Apex integration |
| `references/troubleshooting.md` | Compile/deploy/runtime troubleshooting + LSP loop |
| `references/AccountService.cls` | Service class reference example |
| `references/AccountSelector.cls` | Selector class reference example |
| `references/AccountDeduplicationBatch.cls` | Batch class reference example |
| `assets/*.cls`, `assets/trigger.trigger` | Authoring templates per class/trigger type |
| `hooks/scripts/validate_apex.py` | Apex validation hook |
| `hooks/scripts/apex-lsp-validate.py` | LSP-based syntax validation |
| `hooks/scripts/llm_pattern_validator.py` | LLM anti-pattern detection |
| `hooks/scripts/prettier-format.py` | Apex formatting |
| `hooks/scripts/post-tool-validate.py` | Post-write validation orchestration |
