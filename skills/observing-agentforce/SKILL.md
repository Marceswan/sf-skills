...
description: "Analyze production Agentforce agent behavior using session traces, the Session Tracing Data Model (STDM), and Data Cloud / Data 360 telemetry. TRIGGER when: user queries STDM session data or Data Cloud trace records; extracts session tracing data to Parquet or analyzes it with Polars; investigates production agent failures, regressions, or performance issues; asks about session traces, conversation logs, latency, or agent metrics; wants to reproduce a reported production issue in preview; runs findSessions or trace analysis queries. DO NOT TRIGGER when: user creates, modifies, or debugs .agent files during development (use developing-agentforce); writes or runs test specs (use testing-agentforce); writes Agent Script code (use building-agentscript); uses sf agent preview for local development iteration; reads Apex debug logs (use debugging-apex-logs); deploys or publishes agents."
...
## STDM Extraction and Parquet/Polars Analysis

Beyond the in-org STDM Apex queries used in Phase 1, this skill can extract Session Tracing Data Model records to disk for durable, large-scale analysis.

### Storage / analysis model
- extraction via Data 360 / Data Cloud APIs (see `scripts/`)
- Parquet (via PyArrow) for storage efficiency
- Polars for large-scale lazy analysis

### Core STDM entities
Expect work around: session, interaction / turn, interaction step, moment, message. GenAI Trust Layer / audit records may also be relevant for content-quality and generation debugging. Full schema: `references/stdm-schema.md` and `references/data-model-reference.md`.
...
Use the provided scripts under `scripts/` (`extractor.py`, `analyzer.py`, `datacloud_client.py`, `auth.py`, `cli.py`) rather than reimplementing extraction logic. Auth setup: `references/auth-setup.md`. CLI usage: `references/cli-reference.md`.

### Polars analysis goals
...
See `references/analysis-cookbook.md`, `references/analysis-examples.md`, and `references/polars-cheatsheet.md`.

## High-Signal Operational Rules

- treat STDM as **read-only telemetry**
- expect ingestion lag (about 15 minutes); this is not perfect real-time debugging
...
|------|-------------|--------|
| Auth / JWT / ECA setup | configuring-connected-apps | Data 360 access |
| Fix agent routing / behavior (Agent Script) | building-agentscript | Authoring corrections |
| Fix Builder-based agent / topics / actions | developing-agentforce | Setup UI / metadata corrections |
| Formal regression / coverage tests | testing-agentforce | Reproducible test loops |
| Flow-backed action debugging | generating-flow | Declarative repair |
| Apex-backed action debugging | debugging-apex-logs or generating-apex | Code / log investigation |
...
| `references/data-model-reference.md` | Full STDM entity / DMO schema reference |
...
| `references/basic-extraction.md` | Baseline last-N-days extraction |
| `references/filtered-extraction.md` | Date / agent filtered extraction |
| `references/cli-reference.md` | Extraction CLI usage |
| `references/auth-setup.md` | Data 360 / JWT / ECA auth setup |
| `references/query-patterns.md` | Reusable STDM query patterns |
| `references/client-demo-queries.md` | Demo-ready query set |
| `references/analysis-cookbook.md` | Polars analysis recipes |
| `references/analysis-examples.md` | Worked analysis examples |
| `references/polars-cheatsheet.md` | Polars lazy-analysis cheatsheet |
| `references/debugging-sessions.md` | Single-session debugging procedures |
| `references/agent-execution-lifecycle.md` | Agent execution lifecycle mapped to trace steps |
| `references/builder-trace-api.md` | Builder Trace API extraction |
| `references/billing-and-troubleshooting.md` | Cost considerations and troubleshooting |
| `references/troubleshooting.md` | Troubleshooting table |
| `scripts/` | Extraction / analysis Python package (`extractor.py`, `analyzer.py`, `datacloud_client.py`, `auth.py`, `cli.py`, `models.py`) |
| `apex/AgentforceOptimizeService.cls` | In-org STDM helper Apex class (deploy once per org) |
| `assets/queries/` | Ready SQL for sessions, interactions, messages, steps |
| `assets/analysis/` | Python snippets for session summary, message timeline, step distribution |
