...
# Create External Client App (via configuring-connected-apps skill)
...
For real-time agent trace capture and analysis (v1.1 PlanSuccessResponse with 13 step types), use **testing-agentforce** Phase F via `sf agent preview start/send/end`. This replaces the previous browser-based capture approach with a simpler CLI workflow.
...
| Skill | Use Case |
|-------|----------|
| `configuring-connected-apps` | Set up JWT Bearer auth |
| `building-agentscript` | Fix agents based on trace analysis |
| `testing-agentforce` | Create tests from observed patterns |
| `debugging-apex-logs` | Deep-dive into action failures |
