...
Optional extraction scripts are available in `scripts/` -- see the Reference File Index below. The wrapper `extract_salesforce_doc.py` routes `help.salesforce.com` URLs into the dedicated Help extractor and handles other official doc hosts (`*.salesforce.com`, `lightningdesignsystem.com`) through a generic browser-rendered path. For bot-sensitive pages it supports best-effort stealth mode via `--stealth` when `playwright-stealth` is installed.
...
Other Salesforce skills should delegate to `fetching-salesforce-docs` when they need authoritative Salesforce documentation instead of relying on generic search alone. Prefer official documentation over inference when answering questions about API behavior, release-gated features, or metadata type specifications.
