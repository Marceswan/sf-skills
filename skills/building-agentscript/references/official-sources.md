<!-- Parent: building-agentscript/SKILL.md -->
...
## Local Documentation Index (fetching-salesforce-docs)

All primary reference URLs above have been crawled and indexed locally (2026-04-02). **Search the local index first** before fetching URLs:

```bash
# Semantic search across all indexed Agentforce docs (78 pages)
node ~/.claude/tools/fetching-salesforce-docs/dist/cli.js search "agent script blocks"

# Then Read the file path from the results for full content
```

This is faster and more reliable than fetching live URLs, and works offline.

---

## Fallback Search Patterns

When the local index doesn't have the answer and a specific URL 404s:
