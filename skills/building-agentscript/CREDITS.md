...
| August Krys | Prompt metadata modernization follow-up, including consistency updates around `GenAiPromptTemplate`, published/active template readiness, and current prompt-template examples |
...
- **developing-agentforce** - Standard Agentforce platform skill (Agent Builder, PromptTemplate, Models API)
- **testing-agentforce** - Agent testing with CLI
- **generating-flow** - Flow development for `flow://` action targets
- **deploying-metadata** - Deployment validation and execution
...
| 100-Point Scoring System | sf-skills Pattern (adapted from developing-agentforce) |
